...
import { importTecnicosExcel } from "@/lib/api";
import type { Tecnico } from "@/types/tecnico";

interface TecnicoImportProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImportSuccess: (tecnicos: Tecnico[]) => void;
}

type ImportStatus = "idle" | "uploading" | "success" | "error";

export function TecnicoImport({
  open,
  onOpenChange,
  onImportSuccess,
}: TecnicoImportProps) {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      const tecnicos = await importTecnicosExcel(file);
      setImportedCount(tecnicos.length);
      setStatus("success");
      onImportSuccess(tecnicos);
    } catch (err) {
      setStatus("error");
      setError(
        err instanceof Error
          ? err.message
          : "Erro ao importar técnicos. Tente novamente."
      );
    }
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Importar Técnicos</DialogTitle>
          <DialogDescription>
            Faça upload de um arquivo Excel (.xlsx) com os dados dos técnicos.
            ...
            <div className="flex items-center gap-3 p-4 rounded-lg bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800">
              ...
              <div>
                ...
                <p className="text-sm text-green-600 dark:text-green-400">
                  {importedCount} técnico(s) importado(s) com sucesso.
                </p>
              </div>
            </div>
          )}
            ...
            <>
              ...
              <div className="rounded-lg bg-muted/50 p-3 text-sm">
                ...
                <ul className="space-y-1 text-muted-foreground">
                  ...
                  <li>
                    • <strong>email</strong> (obrigatório)
                  </li>
                  <li>
                    • <strong>endereco</strong> (obrigatório)
                  </li>
                  <li>
                    • <strong>placa</strong> (obrigatório)
                  </li>
                  <li>• especialidade (opcional)</li>
                </ul>
              </div>
            </>
          )}
